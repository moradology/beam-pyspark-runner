...
class PartialGroupByKeyCombiningValues(beam.DoFn):
  """Aggregates values into a per-key cache."""
  def __init__(self, combine_fn):
    self._combine_fn = combine_fn
    self._combined = self._combine_fn.create_accumulator()
  ...
  def process(self, element):
    k, vi = element
    print("ACCUMULATOR:", self._combined)
    print("ELEMENT:", element)
    self._combined = self._combine_fn.add_input(self._combined, vi)
    print("ACCUMULATOR AFTER ADD", self._combined)
    #yield self._combined
